...
public class ContactUtils {
	...
	private String getBirthday(long contactId) {
		...
		if (c.moveToFirst()) {
			...
			birthday = String.valueOf(birthdayCalendar.getTimeInMillis());
		}
		...
		for (Contact c : contacts) {
			...
			if (newData) {
				...
				ops.add(ContentProviderOperation.newInsert(Data.CONTENT_URI)
				          ...
				          .withValue(BirthdayConstant.MONTH, birthdayCalendar.get(Calendar.MONTH))
				          .withValue(BirthdayConstant.DAY, birthdayCalendar.get(Calendar.DAY_OF_MONTH))
				          .build());
			} else {
				ops.add(ContentProviderOperation.newUpdate(Data.CONTENT_URI)
				          .withSelection(Data.RAW_CONTACT_ID + " = ? AND " + Data.MIMETYPE + " = ? ", new String[]{rawContactId.toString(), BirthdayConstant.TYPE})
